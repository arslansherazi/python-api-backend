...
def upload_any_file(request):
    ...
    fs = FileSystemStorage(location='Uploads/AllFormats')
...
def upload_multiple_files(request):
    ...
    fs = FileSystemStorage(location='Uploads/MultipleFiles')
...
def upload_particular_format_file(request):
    ...
    if file_extension_check:
        fs = FileSystemStorage(location='Uploads/ParticularFormats')
        file = fs.save(file.name, file)  # returns name of saved file
        if file:
            request.session['message'] = 'File is uploaded successfully'
        else:
...
def view_files(request, type):
    if type == 'all_formats':
        path = os.path.join(BASE_DIR, 'Uploads\\AllFormats')
        files_dict = make_files_dict(path)
    elif type == 'multiple_files':
        path = os.path.join(BASE_DIR, 'Uploads\\MultipleFiles')
        files_dict = make_files_dict(path)
    elif type == 'specific_formats':
        path = os.path.join(BASE_DIR, 'Uploads\\ParticularFormats')
        files_dict = make_files_dict(path)
    else:
